...
class Hunter(agent_def.Agent):
    ...
    def calculate_super_error(self, angle_front, angle_behind, rho_front, rho_behind, u_front, u_behind, v_front, v_behind, v_target_x, v_target_y):
        ...
        # print(self.angle)
        # print(self.orientation)
        self.z_1 = np.array([[(self.distance - rho_0) ** 2 + (2 * self.angle - angle_front - angle_behind) ** 2 + (self.orientation + pi / 2 - self.angle)]]).reshape(1,1)
        # print(self.z_1)
        # print(self.z_1.shape)

        self.alpha_1 = (self.distance - rho_0) * (np.array([[cos(self.angle), sin(self.angle)]]))
        # print(self.alpha_1)
        ...
        self.gamma_1 = (self.orientation + pi / 2 - self.angle)
        ...
        self.delta = np.dot(np.dot(self.beta_2 , Q_transform_front) , np.array([[u_front, v_front]]).T) + np.dot(np.dot(self.beta_3 , Q_transform_behind) , np.array([[u_behind, v_behind]]).T)
        ...
        self.lambda_2 = 0.1 * np.array([np.dot((self.alpha_1 + self.beta_1 - self.beta_2 - self.beta_3) , np.array([v_target_x, v_target_y]))])
        # print(self.lambda_2.shape)
        #print(np.dot(self.beta_3 , Q_transform_behind , np.array([[u_behind, v_behind]]).T).shape)
        self.dot_z_1 = 0.1 * np.dot(np.dot(self.beta_3 , Q_transform_behind) , np.array([[u_behind, v_behind]]).T) - self.lambda_2
